...
export default {
  ...
  synchronization_definition: {
    ...
    scene_mode: '场景模式',
    scene_mode_first: '请先选择场景模式',
    datasource_tips: '请选择数据源',
    datasource_empty_hint:
      '未找到可用数据源。请先选择「单表同步」场景模式，并确保已在 SeaTunnel 中安装对应连接器插件（如 connector-jdbc）后同步连接器。',
    ...
    schedule_name: '调度名称',
    schedule_name_tips: '请输入调度名称',
    schedule_name_validate: '调度名称不能为空',
    crontab: 'Cron 表达式',
    cron_mechanism: '定时机制',
    crontab_tips: '例如：0 0 2 * * ? 表示每天凌晨 2 点执行',
    crontab_validate: 'Cron 表达式不能为空',
    schedule_type: '调度类型',
    schedule_type_internal: '内置调度',
    schedule_type_dolphin: 'DolphinScheduler',
    schedule_enabled: '启用',
    schedule_delete: '删除调度',
    schedule_save_success: '定时任务保存成功',
    schedule_save_failed: '定时任务保存失败',
    schedule_delete_success: '定时任务已删除',
    schedule_delete_failed: '定时任务删除失败',
    crontab_internal_hint:
      '内置调度使用 Spring Cron（6 位，含秒），例如 0 0 2 * * ? 表示每天 2:00 执行',
    crontab_dolphin_hint:
      'DolphinScheduler 使用 Quartz Cron（7 位），需先在 application.yml 中配置 DS 连接信息',
    cron_field_second: '秒',
    cron_field_minute: '分钟',
    cron_field_hour: '小时',
    cron_field_day: '日',
    cron_field_month: '月',
    cron_field_week: '周',
    cron_mode_every: '每{field}',
    cron_mode_unspecified: '不指定',
    cron_mode_period: '周期 从',
    cron_mode_loop: '循环 从',
    cron_mode_appoint: '指定',
    cron_to: '至',
    cron_loop_from: '开始，每',
    cron_loop_step: '执行一次',
    cron_desc_every: '每{field}',
    cron_desc_period: '{field}从{start}至{end}',
    cron_desc_loop: '{field}从{start}开始，每{step}执行一次',
    cron_desc_appoint: '{field}指定为 {values}',
    cron_desc_default: '自定义 Cron 表达式',
    cron_preview_title: '最近五次执行时间',
    cron_preview_empty: '暂无可预览的执行时间',
    cron_preview_failed: 'Cron 表达式无效，无法预览执行时间',
    cron_week_1: '周日',
    cron_week_2: '周一',
    cron_week_3: '周二',
    cron_week_4: '周三',
